...
| | |
|--|--|
| 已做 | yaw 横条裁帧；36 金币/白板/角标/飞分预铺（不用 `opacity: 0`，不 `-9999` 拆层）；魔法原色不换白板 URL；`warmupLookAssets`；脏格子 rAF（延迟圈才 sync）；翻牌窗口关投影；原色子藏掉后停更新；全盘翻牌不整盘选中浮起；投影松手 100ms 淡入。金币 Additive 0.18（白板不叠）。飞币条数约路径 2/3 |
