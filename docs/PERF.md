...
真机 Safari / WKWebView。桌面流畅不能当过关。日志：`[perf]` 打到 Xcode/Safari 控制台，`?debugPerf=1` 左下角可复制。

## 状态

| | |
|--|--|
| 已做 | yaw 横条裁帧；36 金币/白板/角标/飞分预铺（不用 `opacity: 0`，不 `-9999` 拆层）；魔法原色不换白板 URL；`warmupLookAssets`；脏格子 rAF（延迟圈才 sync）；翻牌窗口关投影；原色子藏掉后停更新；全盘翻牌不整盘选中浮起；投影松手 100ms 淡入。金币 Additive 已按玩法打开（同图拷贝，点魔法/划金币仍可能卡） |
| 未过关 | 点魔法 / 划一长串金币仍可能卡（36 路 yaw + 1.35 + 金币缩放的合成上限）。第一次亮 overlay 仍可能掉帧 |
| 不做 | 整盘 canvas 合成（金币对不齐，已回滚）；砍翻牌动画换流畅 |

对照场景与旧检索记录见下文。再动性能先看 `tap=1` 的 `magic_flip` / `magic_back`。
