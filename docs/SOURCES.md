...
2. [DESIGN.md](./DESIGN.md) 玩法 9×9  
3. [OPERATION.md](./OPERATION.md) 滑动手感  
4. 盘面数字、占坑、重力：TripleMatch 代码（`GameSettings`，不用 Config 的 42）  
5. 指针 / DOM 盘 / rect：SlidetoWord `portrait-webgpu-base/src/game/mount.ts`  
6. 射线 path、三消匹配、刚体物理：不听  
7. 滑动检索：[SWIPE-RESEARCH.md](./SWIPE-RESEARCH.md)（不覆盖规则）  
