...
震动不是 `cap:sync` 自动带上的。Capacitor 默认 VC **不会**注册本地 `AdvancedHaptics`。必须把 Swift 拷进 `ios/App/App/`，并把 storyboard 换成 `BridgeViewController`。这些只发生在 **`npm run ios:bootstrap`**。

### 0.1 三种场景用哪条命令

| 你刚做了什么 | 命令 | 说明 |
|--------------|------|------|
| 刚 clone / 没有 `ios/` / 第一次真机 | `npm run ios:bootstrap` 再 `cap:open` | add ios + 拷插件 + 改 storyboard/pbx + sync |
...
只跑 `npm run ios` / `cap:sync` **不会**修 storyboard。`ios/` 若是 `cap add ios` 的干净工程，插件一定没挂上。
...
3. `ios/App/App/Base.lproj/Main.storyboard` 里入口 VC 是：
...
| `platform: 'ios'` 但 `plugin: false` | **没注册**。回去跑 bootstrap，查 storyboard / Compile Sources |
...
| storyboard 仍是 `CAPBridgeViewController` | `isPluginAvailable === false` | bootstrap 或 Xcode 把 customClass 改成 `BridgeViewController` |
...
| `prepare()` | **空操作**。注释写明：引擎在 Swift `load()` 已起 | `main.ts` **await prepare()**；Swift **没有** `prepare` 方法 |
...
1. 启动就 `await haptics.prepare()`，原生没有这个方法 → 日志 `UNIMPLEMENTED` / `ok: false`。  
   **引擎其实已经在 `plugin.load()` 里起来了。**
2. 只跑了 `cap:sync`、没跑 `ios:bootstrap` → storyboard 仍是默认 `CAPBridgeViewController` → 插件没注册 → `plugin_unavailable`。
