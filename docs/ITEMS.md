...
占格，四向可划，和普通子同一套加/减格。盘上颗数无上限（最多 36）。顶补只出普通色。非魔法划若把盘上该锁定色清光，这一波顶补不出该色；否则色池不改。魔法 / 金币划不禁色，清屏后仍按至少 3 色补。
...
- 从道具起划：锁定色未定，第一颗普通子锁定颜色。点中 / 划到变色子时，其四邻可连子不走其它色变暗（见 FEEDBACK）。
- 锁色改变时，路径上普通子只换贴图为当前锁色（见 FEEDBACK）。回退反向；有效抬手先记下显示色再丢掉翻面（缩没和碎屑用该色）。盘面逻辑色不变。
- **变色散消滑动中不浮起散子，用 Additive 发亮标记。** 松手后角标从 1 号往队尾逐个消失；每少一个角标，队尾数字 -1 并点 1 颗散子。散子选完后剩下的角标 120ms 内收完，到 0 后再停 500ms 与路径一起消。
...
| `src/game/scoreFly.ts` | 魔法路径金币飞向 HUD 图标（条数约 2/3） |
